//document.getElementsByTagName("header")[0].innerHTML = JSON.stringify(dataHouse,null,2)
...
for (var i = 0; i < dataHouse.results[0].members.length; i++) {
    ...
    row.className = dataHouse.results[0].members[i].party + " " + dataHouse.results[0].members[i].state;
    ...
    for (var j = 0; j < 5; j++) {
        ...
        if (j == "0") {
            cell.innerHTML = '<a href="' + dataHouse.results[0].members[i].url + '">' + cellText + '</a>';
        } else {
...
function populate(j) {
    if (j == "0") {
        if (dataHouse.results[0].members[i].middle_name == null) {
            return dataHouse.results[0].members[i].first_name + " " + dataHouse.results[0].members[i].last_name;
        } else {
            return dataHouse.results[0].members[i].first_name + " " + dataHouse.results[0].members[i].middle_name + " " + dataHouse.results[0].members[i].last_name;
        }
    } else if (j == "1") {
        return dataHouse.results[0].members[i].party;
    } else if (j == "2") {
        return dataHouse.results[0].members[i].state;
    } else if (j == "3") {
        return dataHouse.results[0].members[i].seniority;
    } else {
        return dataHouse.results[0].members[i].votes_with_party_pct;
    }
}
...
filterStateSelector.onchange = filterState;
